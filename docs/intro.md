...
### 2. 老舊瀏覽器環境 (有開啟 JS，但瀏覽器不支援新 API)

**✅ 完美相容，套件皆有實作優雅降級（Graceful Degradation）處理。**
*   **原因**：套件內部在呼叫新版 Web API 之前，皆會主動偵測目前瀏覽器的環境支援度。若偵測到不支援，則會自動跳過動畫或特定功能，而不會拋出致命的 JS 錯誤（Uncaught ReferenceError）導致整個網站中斷運作。
*   **表現**：
    *   **路由切換 (A)**：若瀏覽器不支援 `View Transitions API`，則會自動跳過轉場動畫，直接降級為 VitePress 原生的 SPA 單頁應用路由切換。
    *   **深淺色切換 (B)**：若不支援 `View Transitions API`，則會直接切換深淺色外觀，不呈現圓形擴散動畫。
    *   **側邊欄追蹤 (C)**：若瀏覽器不支援 `IntersectionObserver`，套件會在主控台印出警告資訊 `[vitepress-sidebar-anchor] IntersectionObserver is not supported in this browser.` 並直接返回，使側邊欄高亮降級為原生狀態（不進行即時滾動追蹤），確保其餘 JS 邏輯照常運作。
